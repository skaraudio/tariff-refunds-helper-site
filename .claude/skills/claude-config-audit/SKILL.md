...
allowed-tools: Read, Glob, Grep, Write, Edit, Agent, WebFetch(domain:code.claude.com), WebFetch(domain:platform.claude.com), Bash(git *), Bash(gh issue *), Bash(claude plugin validate *), Bash(node *), Bash(find *), Bash(grep *), Bash(wc *), Bash(ls *), Bash(cat *), Bash(mkdir *), Bash(cp *), Bash(rm *), Bash(date *)
model: fable
effort: max
---
...
5. **Fetch current docs before judging anything.** WebFetch https://code.claude.com/docs/en/memory, https://code.claude.com/docs/en/skills, https://code.claude.com/docs/en/sub-agents, https://code.claude.com/docs/en/claude-directory, https://code.claude.com/docs/en/best-practices, and the prompting guide https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5-5 that the repo's Model & prompting conventions section points to. Your training data on frontmatter fields, load order, and size limits is behind. When a rewrite depends on a doc rule, cite the section.
...
7. **Subagents read and write; you orchestrate.** Every worker and grader is a `general-purpose` Agent spawn with its full tool set (Read, Write, Edit, Bash) that reads each of its files in full (an excerpt is not a read) and edits its own slice's files directly. Pass `model: "opus"` on every worker and grader spawn; without it they inherit this run's `fable`, which is reserved for the adjudicator. Handle a large or claim-dense slice by splitting it, not by changing model, and record `slice | model | reason` in `<scratch>/spawns.md`. At most 6 spawns in flight at once; keep independent slices running in parallel. Every spawn happens inside this session, which holds the round count: never a separate top-level session, never a hand-typed "Reviewer A/B/C" or "re-review N of M" prompt.
...
10. **Scratch and state.** Allocate and verify a task directory under `.claude/temp/workspace/tasks/<task-slug>-<UUID>/` at the current Git root, following root `AGENTS.md` §Temporary work (Codex and Claude Code). Use its `notes/` child as `<scratch>` for this audit; never stage scratch. Each phase ends by appending `phase N done <timestamp>` to `<scratch>/state.md`. With `--resume <scratch-dir>` in `$ARGUMENTS`, read `state.md` and continue from the first phase not marked done, after re-running the Phase 0 snapshot to detect drift.
...
A. **Accuracy** — commands, paths, facts exist and are current. Any false claim caps the score at 1. Forward-looking guidance names Claude models only by alias (`opus`; `fable` where the repo designates it), never by version or a lesser tier (Sonnet, Haiku, `opusplan`, versioned IDs); docs describing the application's own model IDs match the code.
...
D. **Specificity** — imperative, concrete, verifiable: "run `pnpm test:unit` before committing", not "test your changes". Vague qualifiers, motivational prose, and "you are an expert" preambles score 0, and so do thinking instructions (the deep-reasoning keyword, "think step by step", "use medium thinking"): depth comes from `effort`.
...
Writing standard:
- First line of each file states what it governs. Headers plus short bullets. Imperatives. One rule per bullet, with its reason when the reason isn't obvious.
- Prompting follows the guide linked from the repo's Model & prompting conventions: hard constraints stated plainly once with their reason, not stacked MUST/NEVER/CRITICAL; completion criteria and the check that proves them, not "be thorough" padding; goals, constraints and done-criteria for judgment work, with numbered steps kept only where order matters (deploys, migrations, git sequences); no thinking instructions.
...
- Agents: description = when to delegate. Body = role, inputs, exact output format, what not to do. `tools:` minimal; `model` and `effort` per criterion F.
...
Nobody is watching this run to answer questions. A message without a tool call ends your turn, and the work stops there until I restart it. While work is still owed, don't end a turn in any of these ways: a summary that announces the next phase instead of starting it; an offer to carry on unless I prefer otherwise; a list of decisions for me when none of them blocks the remaining work (those go to the adjudicator or to "Decisions for me"); or stopping because the run has been long or a phase is done. If you notice yourself inviting me to redirect you or offering to wait, delete it and do the next thing. Put status notes in the same message as your next tool call and continue with whatever doesn't depend on an answer. `<scratch>/state.md` is the checklist: a phase not marked done is unfinished work. If a spawn or command you started is still running, wait for its result before moving on. End the turn only when Phase 5 is complete, when nothing can move without me, when the blocker is deliberately protected from you, or when the Phase 4 stop condition or three-round cap ends the review (then go straight to Phase 5 and report what remains). This does not override the limits above: never push, never create a branch, and leave any destructive or out-of-scope action undone and listed under "Decisions for me".
